...
standalone HTML research report. It consumes the [aggregation request](aggregation.md)
...
```sh
node dist/src/cli.js atlas build atlas.json atlas-output
node dist/src/cli.js atlas serve atlas.json
```
...
```sh
node dist/src/cli.js atlas build atlas.json selected-report \
...
node dist/src/cli.js atlas build .ebo/atlas-fixture/atlas.json .ebo/atlas-report
node dist/src/cli.js atlas serve .ebo/atlas-fixture/atlas.json
...
- COE-586 contributed: PR #41: feat(atlas): add Grafana cohort exploration and evidence reports (merge `2fecc0d`)
