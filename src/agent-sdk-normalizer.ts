...
export async function readQualifiedClaudeAgentSdkCapture(
  ...
  if (!["agent-sdk", CLAUDE_AGENT_SDK_HARNESS].includes(manifest.run.harness.id)
      || !manifest.run.runtime.some(({ source, name }) => source === "anthropic" && ["agent-sdk", CLAUDE_AGENT_SDK_HARNESS].includes(name))) {
