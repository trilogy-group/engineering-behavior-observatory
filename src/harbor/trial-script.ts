/** Harbor owns the trial. This extension only installs/executes the native TS worker. */
...
class EboAgent(BaseAgent):
    def __init__(self, *args, ebo_request=None, **kwargs):
        ...
        self.request = json.loads(Path(ebo_request).read_text())
        self.index = 0
    ...
    async def setup(self, environment):
        ...
        if hashlib.file_digest(archive.open('rb'), 'sha256').hexdigest() != runtime['archiveDigest']:
            raise ValueError('Worker runtime archive changed')
        await environment.upload_file(archive, '/tmp/ebo-worker-runtime.tgz')
        # A digest-pinned operator runtime, never a task-supplied archive.
        result = await environment.exec('mkdir -p /tmp/ebo-worker && tar -xzf /tmp/ebo-worker-runtime.tgz -C /tmp/ebo-worker')
        if result.return_code: raise RuntimeError('Worker runtime extraction failed: ' + (result.stderr or ''))
    ...
    async def run(self, instruction, environment, context):
        ...
        runtime = self.request['runtime']
        argv = ['/tmp/ebo-worker/' + runtime['node'], '/tmp/ebo-worker/' + runtime['entrypoint'], root + '/input.json']
        ...
        try:
            ...
            # A canceled Docker exec may leave the Node process running. Bound its drain
...
async def main(request_path):
    ...
    config = TrialConfig.model_validate({
        ...
        'agent': {'import_path': module + ':EboAgent', 'model_name': request['model'],
                  'kwargs': {'ebo_request': request_path}, 'resume_trajectory': False},
