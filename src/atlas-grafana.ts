...
export function atlasDashboards(view: AtlasView, atlasUrl = view.atlasUrl ?? "http://127.0.0.1:13011") {
  ...
  const target = (path: string, columns: string[]) => ({ refId: "A", datasource, type: "json", source: "url", url: `${atlasUrl}${path}?${query}`, url_options: { method: "GET", data: "", headers: [], params: [] }, parser: "backend", format: "table", root_selector: "", columns: columns.map((selector) => ({ selector, text: selector, type: ["numerator", "denominator", "count"].includes(selector) ? "number" : "string" })) });
  return ["overview", "behavior"].map((kind) => ({
    ...
    panels: [
      ...
      { id: 2, type: "table", title: kind === "overview" ? "Selected cohort metrics · exact counts and exclusions" : "Judge-assessed behavior distributions · opposite assessments remain separate", gridPos: { x: 0, y: 4, w: 24, h: 13 }, datasource,
