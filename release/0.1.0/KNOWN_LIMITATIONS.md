...
| OpenHands Agent Server `1.46.0` | Conversation endpoints and consumed event schemas match 1.44.1; contract tests cover current execution and historical retained evidence. The in-process EventLog completeness gap remains explicit. | Native macOS ARM64 binary installed with upstream SHA-256 verification; `/server_info` reports server/SDK/tools/workspace 1.46.0. Model-backed conversation capture still requires a selected model route and has not been verified in this upgrade. |
